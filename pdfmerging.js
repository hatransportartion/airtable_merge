...
async function mergePDFs(pdfPaths, outputPath) {
    ...
    for (const pdfPath of pdfPaths) {
        ...
        const imageBytes = await fetch(pdfPath.url).then((res) => res.arrayBuffer());

        if ( pdfPath.filetype === 'application/pdf' || pdfPath.filetype === 'image/pdf') {
        ...
        } else if (pdfPath.filetype === 'image/jpeg' || pdfPath.filetype === 'image/jpg') {
        ...
        } else if (pdfPath.filetype === 'image/png' || pdfPath.filetype === 'image/png') {
            ...
            console.error('Unsupported file type:', fileExtension, " & ", pdfPath);
            continue;
        }
